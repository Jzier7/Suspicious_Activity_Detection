...
DATASET_DIR = "../../../../../../media/jzier/Elements/DATASETS/final_dataset/"
...
output_directory = '../weights_and_results/improved1/'
...
lrcn_training_history = lrcn_model.fit(x=features_train, y=labels_train, epochs=epochs, batch_size=batch_size,
                                       shuffle=True, validation_split=validation_split, callbacks=callbacks)

# Evaluate the LRCN model on test data
...
train_size = len(features_train) * (1 - validation_split)
val_size = len(features_train) * validation_split
...
with open(os.path.join(output_directory, 'model_metrics.txt'), 'w') as metrics_file:
    ...
    metrics_file.write("\nTraining Details:\n")
    metrics_file.write(f"Epochs: {epochs}\n")
    metrics_file.write(f"Batch Size: {batch_size}\n")
    metrics_file.write(f"Learning Rate: Adam default\n")
    metrics_file.write(f"Optimization Tool: Adam\n")
    metrics_file.write("\nDataset Split:\n")
    metrics_file.write(f"Training Data: {train_percentage:.2f}%\n")
    metrics_file.write(f"Validation Data: {val_percentage:.2f}%\n")
    metrics_file.write(f"Test Data: {test_percentage:.2f}%\n")
